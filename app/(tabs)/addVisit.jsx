...
//     setTasksItems(
//       tasks.map((task) => ({
//         label: task.taskName || 'Unnamed Task', // Add fallback
//         value: task.id,
//       }))
//     );
//   }
...
//   status: parseInt(statusType), // Convert string back to number
//   priority: 0,
//   notes,
//   amountReceived: parseFloat(amountReceivedRaw || "0"),
//   taskDto: selectedTasks.map((taskId) => ({
//     taskId: taskId,
//     taskName: tasksItems.find((t) => t.value === taskId)?.label || "Unknown Task",
//   })),
// };
...
//     {String(clientsError?.includes("Network") || tasksError?.includes("Network")
//       ? "تعذر الاتصال بالخادم. تحقق من اتصال الإنترنت."
//       : clientsError || tasksError || visitError || "حدث خطأ غير معروف")}
//   </Text>
// )}
...
// <View style={[styles.pickerContainer, { zIndex: openTasks ? 3000 : 1000 }]}>
...
//     setValue={() => {}} // We handle this in onSelectItem
//     setItems={setTasksItems}
//     searchable={true}
//     placeholder="حدد المهمة"
//     listMode="MODAL"
//     modalProps={{ animationType: "slide" }}
//     style={styles.dropdown}
//     dropDownContainerStyle={styles.dropdownContainer}
//     loading={tasksLoading}
//     searchablePlaceholder="ابحث عن المهمة"
//     textStyle={{ fontFamily: 'Cairo-Medium' }}
//     containerStyle={{ direction }}
//     onSelectItem={(item) => {
//       if (item && !selectedTasks.includes(item.value)) {
//         setSelectedTasks([...selectedTasks, item.value]);
//       }
//       setOpenTasks(false);
//     }}
//   />
...
//   const task = tasksItems.find((t) => t.value === taskId);
//   if (!taskId || !task || !task.label) {
//     return null;
//   }
//     return (
//       <View   key={taskId} 
//             style={[styles.taskLabel, { flexDirection: direction === 'rtl' ? 'row-reverse' : 'row' }]}
// >
//         <Text style={[styles.taskLabelText, { textAlign }]}>
//           {String(task.label)}
//         </Text>
//         <TouchableOpacity
...
// <View style={[styles.pickerContainer, { zIndex: openStatus ? 2000 : 1000 }]}>
...
<View style={[styles.pickerContainer, { zIndex: openTasks ? 3000 : 1000 }]}>
  ...
  {/* Selected Tasks Display - FIXED: Added proper key prop */}
  {selectedTasks.map((taskId) => {
    ...
    return (
      <View   
        key={`selected-task-${taskId}`} // FIXED: Added unique key
        style={[styles.taskLabel, { flexDirection: direction === 'rtl' ? 'row-reverse' : 'row' }]}
      >
        <Text style={[styles.taskLabelText, { textAlign }]}>
          {String(task.label)}
